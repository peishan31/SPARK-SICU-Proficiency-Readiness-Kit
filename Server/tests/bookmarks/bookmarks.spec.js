...
const getBookmarks = async (userId) => {
    const response = await axios.get(`http://localhost:8080/user/${userId}/bookmarks`);
    return response;
};

const getAllSubchaptersForAChapter = async (userId, chapterId) => {
    const response = await axios.get(`http://localhost:8080/user/${userId}/bookmarks/chapters/${chapterId}`);
    return response;
};

const addBookmarkForASubchapter = async (userId, data) => {
    const response = await axios.put(`http://localhost:8080/user/${userId}/bookmarks/`, data);
    return response;
};

const deleteBookmarkForASubchapter = async (userId, bookmarkId) => {
    const response = await axios.delete(`http://localhost:8080/user/${userId}/bookmarks/${bookmarkId}`);
    return response;
};
...
describe("Testing Add Bookmark to User By UserId route", () => {
    var addedBookmarkId = "";
